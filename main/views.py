...
from .templatetags.custom_filter import get_item
from django.db.models import Q
# algo
from .filter import col_filter
from .test_filter import coll_filter
import logging


#run_filter
def test_filter(request):
    # Memanggil fungsi col_filter dengan parameter yang sesuai (misalnya, "CustomerID")
    recommendations = coll_filter(customerID=13047.0)
    print(recommendations)


    # Mengembalikan hasil dalam respons HTTP
    response = f"Recommendations: {recommendations}"
...
def home(request):
    ...
    if request.user.id is not None:
        user_orders = CartOrderItems.objects.filter(order__user=request.user)
        if user_orders.exists():
            recommended = col_filter(request.user.id)

    # Menghitung rata-rata rating untuk produk yang direkomendasikan
    recommended = Product.objects.filter(title__in=recommended).annotate(avg_rating=Avg('productreview__review_rating')).order_by('-id')

    # Menghitung rata-rata rating untuk produk yang ditampilkan
    featured = Product.objects.filter(is_featured=True).annotate(avg_rating=Avg('productreview__review_rating')).order_by('-id')

    return render(request, 'index.html', {'recommended_product': recommended, 'featured_product': featured, 'banners': banners})
...
def product_list(request):
    ...
    avg_reviews = ProductReview.objects.values('product__id').annotate(avg_rating=Avg('review_rating'))
    avg_review_dict = {review['product__id']: review['avg_rating'] for review in avg_reviews}
    return render(request, 'product_list.html', {'data': data, 'total_data': total_data, 'min_price': min_price, 'max_price': max_price, 'avg_reviews': avg_review_dict})
# Product List According to Category
def category_product_list(request, cat_id):
    category = Category.objects.get(id=cat_id)
    products = Product.objects.filter(category=category).order_by('-id')
    # Mendapatkan rata-rata rating untuk setiap produk
    avg_ratings = {}
    for product in products:
        avg_rating = ProductReview.objects.filter(product=product).aggregate(avg_rating=Avg('review_rating'))['avg_rating']
        if avg_rating is None:
            avg_rating = 0
        avg_ratings[product.id] = avg_rating

    return render(request, 'category_product_list.html', {'data': products,'avg_ratings': avg_ratings,})
...
def brand_product_list(request,brand_id):
	...
	for product in products:
		...
		if avg_rating is None:
			avg_rating = 0
		avg_ratings[product.id] = avg_rating
	return render(request,'brand_product_list.html',{'data':products,'avg_ratings': avg_ratings,})
...
def signup(request):
	if request.method=='POST':
		...
		if form.is_valid():
			...
			print(form.errors)
			return render(request, 'registration/signup.html',{'form':form, 'messages':form.errors})
	form=SignupForm
